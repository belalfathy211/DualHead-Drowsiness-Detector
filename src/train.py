...
eye_train_dataloader = DataLoader(
    eye_train_dataset, batch_size=32, shuffle=True, num_workers=8
)
...
mouth_train_dataloader = DataLoader(
    mouth_train_dataset, batch_size=32, shuffle=True, num_workers=8
)
...
for epoch in range(EPOCHS):
    ...
    for (Xe, ye), (Xm, ym) in zip(eye_train_dataloader, mouth_train_dataloader):
        ...
        train_total_correct_eye += (
            ((y_pred_eye[0] > 0.5).int() == ye.unsqueeze(-1)).sum().item()
        )
        train_total_correct_mouth += (
            ((y_pred_mouth[1] > 0.5).int() == ym.unsqueeze(-1)).sum().item()
        )
    print(
    ...
    with torch.inference_mode():
        ...
        for X, y in eye_val_dataloader:
            ...
            val_loss += loss.item()
            total_correct += ((y_pred[0] > 0.5).int() == y.unsqueeze(-1)).sum().item()
        print(
            f"Eye Val Loss: {val_loss / len(eye_val_dataloader):.4f} | Eye Val Acc: {(total_correct / len(eye_val_dataset) * 100):.2f}%"
        )
        val_loss = 0
        total_correct = 0
        for X, y in mouth_val_dataloader:
            ...
            val_loss += loss.item()
            total_correct += ((y_pred[1] > 0.5).int() == y.unsqueeze(-1)).sum().item()
        print(
            f"Mouth Val Loss: {val_loss / len(mouth_val_dataloader):.4f} | Mouth Val Acc: {(total_correct / len(mouth_val_dataset) * 100):.2f}%"
        )
